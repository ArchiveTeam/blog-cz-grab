...
allowed = function(url, parenturl, force)
  ...
  local match = string.match(url, "^https?://([^%.]+)%.blog%.cz/")
  if match and match == item_value then
    if parenturl and string.match(parenturl, "^https?://[^/]+/[0-9][0-9][0-9][0-9]/[^/]+/?$") then
      return false
    end
    return true
  elseif match then
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  if allowed(url, nil) and status_code == 200
    ...
    html = read_file(file)
    html = string.gsub(html, 'initImageErrorHandler%("[^"]+"%)', "")
    html = string.gsub(html, '<form%s+id="ublog_CommentForm"[^>]+>', "")
    if string.match(url, "^https?://[^%.]+%.blog%.cz/$") then
